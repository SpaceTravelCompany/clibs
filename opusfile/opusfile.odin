//odin binding from https://github.com/xiph/opusfile
package opusfile
...
op_decode_cb_func :: #type proc "c" (
	_ctx: rawptr,
	_decoder: opus.OpusMSDecoder,
	_pcm: rawptr,
	_op: ^ogg.ogg_packet,
	_nsample: c.int,
	_nchannels: c.int,
...
foreign lib {
   ...
   \retval #OP_EBADHEADER If the contents of the packet otherwise violate the
	...
	@(require_results)
	opus_head_parse :: proc(_head: OpusHead, _data: [^]byte, _len: c.size_t) -> c.int ---
	...
	@(require_results)
	opus_tags_parse :: proc(_tags: OpusTags, _data: [^]byte, _len: c.size_t) -> c.int ---
    ...
    \retval #OP_EFAULT     There was not enough memory to store the picture tag
                            contents.*/
	@(require_results)
	opus_picture_tag_parse :: proc(_pic: OpusPictureTag, _tag: cstring) -> c.int ---
   ...
   \return A stream handle to use with the callbacks, or <code>NULL</code> on
            error.*/
	@(require_results)
	op_fopen :: proc(_cb: ^OpusFileCallbacks, _path: cstring, _mode: cstring) -> rawptr ---
	...
	/**Opens a stream with <code>fdopen()</code> and fills in a set of callbacks
      ...
      \return A stream handle to use with the callbacks, or <code>NULL</code> on
                error.*/
	@(require_results)
	op_fdopen :: proc(_cb: ^OpusFileCallbacks, _fd: c.int, _mode: cstring) -> rawptr ---
	...
	/**Opens a stream with <code>freopen()</code> and fills in a set of callbacks
      ...
      \return A stream handle to use with the callbacks, or <code>NULL</code> on
                error.*/
	@(require_results)
	op_freopen :: proc(_cb: ^OpusFileCallbacks, _path: cstring, _mode: cstring, _stream: rawptr) -> rawptr ---
	...
	/**Creates a stream that reads from the given block of memory.
      ...
      \return A stream handle to use with the callbacks, or <code>NULL</code> on
                error.*/
	@(require_results)
	op_mem_stream_create :: proc(_cb: ^OpusFileCallbacks, _data: [^]byte, _size: c.size_t) -> rawptr ---
	...
	/**Creates a stream that reads from the given URL.
      ...
      \return A stream handle to use with the callbacks, or <code>NULL</code> on
                error.*/
	@(require_results)
	op_url_stream_vcreate :: proc(_cb: ^OpusFileCallbacks, _url: cstring, _ap: ^c.va_list) -> rawptr ---
	...
	/**Creates a stream that reads from the given URL.
      ...
      \return A stream handle to use with the callbacks, or <code>NULL</code> on
                error.*/
	@(require_results)
	op_url_stream_create :: proc(_cb: ^OpusFileCallbacks, _url: cstring, #c_vararg args: ..any) -> rawptr ---
	...
	@(require_results)
	op_open_file :: proc(_path: cstring, _error: ^c.int) -> ^OggOpusFile ---
	...
	/**Open a stream from a memory buffer.
      ...
      \return A freshly opened \c ^OggOpusFile, or <code>NULL</code> on error.*/
	@(require_results)
	op_open_memory :: proc(_data: [^]byte, _size: c.size_t, _error: ^c.int) -> ^OggOpusFile ---
	...
	/**Open a stream from a URL.
      ...
      \return A freshly opened \c ^OggOpusFile, or <code>NULL</code> on error.*/
	@(require_results)
	op_vopen_url :: proc(_url: cstring, _error: ^c.int, _ap: ^c.va_list) -> ^OggOpusFile ---
	...
	/**Open a stream from a URL.
      ...
      \return A freshly opened \c ^OggOpusFile, or <code>NULL</code> on error.*/
	@(require_results)
	op_open_url :: proc(_url: cstring, _error: ^c.int, #c_vararg args: ..any) -> ^OggOpusFile ---
	...
	@(require_results)
	op_open_callbacks :: proc(_stream: rawptr, _cb: ^OpusFileCallbacks, _initial_data: [^]byte, _initial_bytes: c.size_t, _error: ^c.int) -> ^OggOpusFile ---
	...
	@(require_results)
	op_test_file :: proc(_path: cstring, _error: ^c.int) -> ^OggOpusFile ---
	...
	/**Partially open a stream from a memory buffer.
      ...
      \return A partially opened \c ^OggOpusFile, or <code>NULL</code> on error.*/
	@(require_results)
	op_test_memory :: proc(_data: [^]byte, _size: c.size_t, _error: ^c.int) -> ^OggOpusFile ---
	...
	/**Partially open a stream from a URL.
      ...
      \return A partially opened \c ^OggOpusFile, or <code>NULL</code> on error.*/
	@(require_results)
	op_vtest_url :: proc(_url: cstring, _error: ^c.int, _ap: ^c.va_list) -> ^OggOpusFile ---
	...
	/**Partially open a stream from a URL.
      ...
      \return A partially opened \c ^OggOpusFile, or <code>NULL</code> on error.*/
	@(require_results)
	op_test_url :: proc(_url: cstring, _error: ^c.int, #c_vararg args: ..any) -> ^OggOpusFile ---
	...
	/**Partially open a stream using the given set of callbacks to access it.
      ...
      \return A partially opened \c ^OggOpusFile, or <code>NULL</code> on error.
              ...
              The calling application is responsible for closing the stream if
                this call returns an error.*/
	@(require_results)
	op_test_callbacks :: proc(_stream: rawptr, _cb: ^OpusFileCallbacks, _initial_data: [^]byte, _initial_bytes: c.size_t, _error: ^c.int) -> ^OggOpusFile ---
   ...
   \retval #OP_EINVAL The \a _gain_type was unrecognized.*/
    ...
    \retval #OP_EBADTIMESTAMP An unseekable stream encountered a new link with
                               ...
                               checks.*/
	@(require_results)
	op_read :: proc(_of: ^OggOpusFile, _pcm: [^]opus.opus_int16, _buf_size: c.int, _li: ^c.int) -> c.int ---
    ...
    \retval #OP_EBADTIMESTAMP An unseekable stream encountered a new link with
                               ...
                               checks.*/
	@(require_results)
	op_read_float :: proc(_of: ^OggOpusFile, _pcm: [^]c.float, _buf_size: c.int, _li: ^c.int) -> c.int ---
    ...
    \retval #OP_EBADTIMESTAMP An unseekable stream encountered a new link with
                               ...
                               checks.*/
	@(require_results)
	op_read_stereo :: proc(_of: ^OggOpusFile, _pcm: [^]opus.opus_uint16, _buf_size: c.int) -> c.int ---
    ...
    \retval #OP_EBADTIMESTAMP An unseekable stream encountered a new link with
                               ...
                               checks.*/
	@(require_results)
	op_read_float_stereo :: proc(_of: ^OggOpusFile, _pcm: [^]c.float, _buf_size: c.int) -> c.int ---
}
