...
foreign lib {
	...
	*/
	...
	initStream :: proc(stateBuffer: rawptr, size: c.size_t) -> ^stream_t ---
...
foreign lib {
	...
	initStreamHC :: proc(buffer: rawptr, size: c.size_t) -> ^streamHC_t ---
...
errorCode_t :: c.size_t

blockSizeID_t :: enum c.int {
	default  = 0,
	max64KB  = 4,
	max256KB = 5,
	max1MB   = 6,
...
blockMode_t :: enum c.int {
	blockLinked      = 0,
	blockIndependent = 1,
}

contentChecksum_t :: enum c.int {
	noContentChecksum      = 0,
	contentChecksumEnabled = 1,
}

blockChecksum_t :: enum c.int {
	noBlockChecksum      = 0,
	blockChecksumEnabled = 1,
}

frameType_t :: enum c.int {
	frame          = 0,
	skippableFrame = 1,
}
...
frameInfo_t :: struct {
	...
	contentSize:         c.ulonglong,
	dictID:              c.uint,
	blockChecksumFlag:   blockChecksum_t,
}

preferences_t :: struct {
	frameInfo:        frameInfo_t,
...
compressOptions_t :: struct {
	stableSrc: c.uint,
	reserved:  [3]c.uint,
}

decompressOptions_t :: struct {
	stableDst:     c.uint,
...
foreign lib {
	LZ4F_isError :: proc(code: errorCode_t) -> c.uint ---
	LZ4F_getErrorName :: proc(code: errorCode_t) -> cstring ---

	LZ4F_compressFrame :: proc(dstBuffer: rawptr, dstCapacity: c.size_t, srcBuffer: rawptr, srcSize: c.size_t, preferencesPtr: ^preferences_t) -> c.size_t ---
	LZ4F_compressFrameBound :: proc(srcSize: c.size_t, preferencesPtr: ^preferences_t) -> c.size_t ---
	LZ4F_compressionLevel_max :: proc() -> c.int ---

	LZ4F_getVersion :: proc() -> c.uint ---

	LZ4F_createCompressionContext :: proc(cctxPtr: ^^LZ4F_cctx, version: c.uint) -> errorCode_t ---
	LZ4F_freeCompressionContext :: proc(cctx: ^LZ4F_cctx) -> errorCode_t ---

	LZ4F_compressBegin :: proc(cctx: ^LZ4F_cctx, dstBuffer: rawptr, dstCapacity: c.size_t, prefsPtr: ^preferences_t) -> c.size_t ---
	LZ4F_compressBound :: proc(srcSize: c.size_t, prefsPtr: ^preferences_t) -> c.size_t ---
	LZ4F_compressUpdate :: proc(cctx: ^LZ4F_cctx, dstBuffer: rawptr, dstCapacity: c.size_t, srcBuffer: rawptr, srcSize: c.size_t, cOptPtr: ^compressOptions_t) -> c.size_t ---
	LZ4F_flush :: proc(cctx: ^LZ4F_cctx, dstBuffer: rawptr, dstCapacity: c.size_t, cOptPtr: ^compressOptions_t) -> c.size_t ---
	LZ4F_compressEnd :: proc(cctx: ^LZ4F_cctx, dstBuffer: rawptr, dstCapacity: c.size_t, cOptPtr: ^compressOptions_t) -> c.size_t ---

	LZ4F_createDecompressionContext :: proc(dctxPtr: ^^LZ4F_dctx, version: c.uint) -> errorCode_t ---
	LZ4F_freeDecompressionContext :: proc(dctx: ^LZ4F_dctx) -> errorCode_t ---

	LZ4F_headerSize :: proc(src: rawptr, srcSize: c.size_t) -> c.size_t ---
	LZ4F_getFrameInfo :: proc(dctx: ^LZ4F_dctx, frameInfoPtr: ^frameInfo_t, srcBuffer: rawptr, srcSizePtr: ^c.size_t) -> c.size_t ---
	LZ4F_decompress :: proc(dctx: ^LZ4F_dctx, dstBuffer: rawptr, dstSizePtr: ^c.size_t, srcBuffer: rawptr, srcSizePtr: ^c.size_t, dOptPtr: ^decompressOptions_t) -> c.size_t ---
	LZ4F_resetDecompressionContext :: proc(dctx: ^LZ4F_dctx) ---

	LZ4F_compressBegin_usingDict :: proc(cctx: ^LZ4F_cctx, dstBuffer: rawptr, dstCapacity: c.size_t, dictBuffer: rawptr, dictSize: c.size_t, prefsPtr: ^preferences_t) -> c.size_t ---
	LZ4F_decompress_usingDict :: proc(dctxPtr: ^LZ4F_cctx, dstBuffer: rawptr, dstSizePtr: ^c.size_t, srcBuffer: rawptr, srcSizePtr: ^c.size_t, dict: rawptr, dictSize: c.size_t, decompressOptionsPtr: ^decompressOptions_t) -> c.size_t ---

	LZ4F_createCDict :: proc(dictBuffer: rawptr, dictSize: c.size_t) -> ^LZ4F_CDict ---
	LZ4F_freeCDict :: proc(cdict: ^LZ4F_CDict) ---
	LZ4F_compressFrame_usingCDict :: proc(cctx: ^LZ4F_cctx, dst: rawptr, dstCapacity: c.size_t, src: rawptr, srcSize: c.size_t, cdict: ^LZ4F_CDict, preferencesPtr: ^preferences_t) -> c.size_t ---
	LZ4F_compressBegin_usingCDict :: proc(cctx: ^LZ4F_cctx, dstBuffer: rawptr, dstCapacity: c.size_t, cdict: ^LZ4F_CDict, prefsPtr: ^preferences_t) -> c.size_t ---
}
