...
Realloc_Func :: #type proc "c" (
	memory: Memory,
	cur_size, new_size: c.long,
	block: rawptr,
) -> rawptr
...
Face_Get_Advances_Func :: #type proc "c" (
	face: Face,
	first, count: c.uint,
	flags: c.int,
	advances: ^Fixed,
...
SpanFunc :: #type proc "c" (y: c.int, count: c.int, spans: ^Span, user: rawptr)

Raster_BitTest_Func :: #type proc "c" (x: c.int, y: c.int, user: rawptr) -> c.int

Raster_BitSet_Func :: #type proc "c" (x: c.int, y: c.int, user: rawptr)
...
Raster_NewFunc :: #type proc "c" (memory: rawptr, raster: ^Raster) -> c.int

Raster_DoneFunc :: #type proc "c" (raster: Raster)

Raster_ResetFunc :: #type proc "c" (raster: Raster, pool: [^]u8, pool_size: u32)

Raster_SetModeFunc :: #type proc "c" (raster: Raster, mode: u32, args: rawptr) -> c.int

Raster_RenderFunc :: #type proc "c" (raster: Raster, params: ^Raster_Params) -> c.int
...
foreign freetype {
	...
	get_next_char :: proc(face: Face, code: c.ulong, agindex: ^c.uint) -> c.ulong ---
	...
	select_size :: proc(face: Face, strike_index: c.int) -> Error ---
	...
	outline_new :: proc(library: Library, numPoints: c.uint, numContours: c.int, anoutline: ^Outline) -> Error ---

	@(link_name = "FT_Outline_Done")
	outline_done :: proc(library: Library, outline: ^Outline) -> Error ---

	@(link_name = "FT_Outline_Check")
	outline_check :: proc(_outline: ^Outline) -> Error ---

	@(link_name = "FT_Outline_Get_CBox")
	outline_get_cbox :: proc(_outline: ^Outline, _acbox: ^BBox) ---

	@(link_name = "FT_Outline_Translate")
	outline_translate :: proc(_outline: ^Outline, _xOffset: Pos, _yOffset: Pos) ---

	@(link_name = "FT_Outline_Copy")
	outline_copy :: proc(_source: ^Outline, _target: ^Outline) -> Error ---

	@(link_name = "FT_Outline_Transform")
	outline_transform :: proc(_outline: ^Outline, _matrix: ^Matrix) ---

	@(link_name = "FT_Outline_Embolden")
	outline_embolden :: proc(outline: ^Outline, strength: Pos) -> Error ---

	@(link_name = "FT_Outline_EmboldenXY")
	outline_embolden_xy :: proc(outline: ^Outline, xstrength: Pos, ystrength: Pos) -> Error ---

	@(link_name = "FT_Outline_Reverse")
	outline_reverse :: proc(outline: ^Outline) ---

	@(link_name = "FT_Outline_Get_Bitmap")
	outline_get_bitmap :: proc(library: Library, outline: ^Outline, abitmap: ^Bitmap) -> Error ---

	@(link_name = "FT_Outline_Render")
	outline_render :: proc(library: Library, outline: ^Outline, params: ^Raster_Params) -> Error ---

	@(link_name = "FT_Outline_Get_Orientation")
	outline_get_orientation :: proc(outline: ^Outline) -> Orientation ---

	@(link_name = "FT_Outline_Decompose")
	outline_decompose :: proc(outline: ^Outline, funcs: ^Outline_Funcs, user_data: rawptr) -> Error ---
}
